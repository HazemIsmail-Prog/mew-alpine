...
<div x-data="{
    ...
    <div id="{{ $id }}" class="w-full overflow-hidden cursor-pointer"
        x-on:resize.window="updateListWidth"
        x-on:keydown.window="if ($event.key === 'Tab' || $event.key === 'Escape') expanded = false"
        x-on:click.outside="expanded = false">
        <div x-show="!expanded" x-on:click="expanded = true" class="flex h-[42px] items-center gap-1 bg-white p-1 border border-primary rounded-lg">
            ...
            <template x-if="!multiple && selectedItems">
                <span class="text-center w-full"
                    x-text="list.find(item => item.id === selectedItems)?.name"></span>
            </template>
        ...
        <div x-cloak x-anchor.bottom-start-end="$refs.button" x-show="expanded" x-ref="list" class="z-20 bg-white border border-primary rounded-lg shadow-md overflow-clip">
            <input class="w-full p-2 border-none rounded-md focus:outline-none focus:ring-0 outline-none"
                type="text" x-model="searchTerm" x-ref="searchTermRef" />
            <div tabindex="-1" class="bg-white border-t border-primary p-2 max-h-[200px] overflow-y-auto no-scrollbar">
                <template x-for="item in sortedItems" :key="item.id">
                    <div x-bind:class="isSelected(item) ? 'bg-blue-200' : 'hover:bg-primary hover:bg-opacity-10'"
                        x-on:click="addItem(item)" class="select-none cursor-pointer p-1"
