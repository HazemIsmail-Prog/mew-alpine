<div wire:key="user-{{ $user->id }} - {{ rand() }}" class=" flex items-center gap-3">

    <div>
        <label x-on:change="toggleUserIsActive({{ $user }},$event.target.checked)"
            class="hidden lg:inline-flex items-center cursor-pointer">
            <input @checked($user->is_active) type="checkbox" class="hidden peer">
            <div
    ...
    <div x-on:click="openUserForm({{ $user }})"
        x-bind:class="[
        ...
        <div>
            ...
            <div class=" font-extrabold text-lg whitespace-pre-line">{!! $user->name !!}
    ...
    <div class="flex flex-col items-center gap-3 my-3 text-xs">
        <button wire:confirm="Sure?" wire:click="deleteUser({{ $user }})" type="button"
