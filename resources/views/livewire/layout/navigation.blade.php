...
<nav x-data="{ open: false }" class="bg-primary bg-opacity-5 print:hidden">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-primary bg-primary bg-opacity-5 hover:text-primary focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @endcan
            @can('viewAny', App\Models\Stakeholder::class)
                <x-responsive-nav-link :href="route('stakeholders.index')" :active="request()->routeIs('stakeholders.index')">
                    {{ __('Stakeholders') }}
                </x-responsive-nav-link>
            @endcan
            @can('viewAny', App\Models\Tag::class)
