@can('createSteps', App\Models\Document::class)
    <div class="flex gap-2 flex-col mt-2">
        <div class="flex items-center gap-2">
            <textarea 
                rows="2" 
                class="w-full focus:outline-none focus:ring-0 border-none outline-none"
                x-ref="newStepActionTextarea" 
                x-model="stepForm.action" 
                x-on:keydown.ctrl.enter.prevent="createNewStep"
            ></textarea>

            <x-svg.save
                class="text-primary size-4" 
                x-show="stepForm.action" 
                x-on:click="createNewStep" 
            />

            <x-svg.cancel 
                class="text-red-500 size-4" 
                x-show="stepForm.action" 
                x-on:click="stepForm.action = ''" 
            />
